...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    if (req.admin.role === 'super_admin') {
      const totalRevenue = await Car.sum('price', { where: { sold: true } }) || 0;
      const monthlyRevenue = await Car.sum('price', {
        where: { 
          sold: true,
  ...
  try {
    ...
    const carsByMake = await Car.findAll({
      attributes: [
        'make',
        [fn('COUNT', col('id')), 'count'],
        [fn('SUM', literal('CASE WHEN sold = true THEN 1 ELSE 0 END')), 'soldCount']
      ],
      group: ['make'],
      order: [[fn('COUNT', col('id')), 'DESC']],
      limit: 10
...
export const getRevenueStats = async (req, res) => {
  try {
    ...
    const monthlyRevenue = await Car.findAll({
      ...
      where: { 
        sold: true,
        updatedAt: { [Op.gte]: new Date(Date.now() - 12 * 30 * 24 * 60 * 60 * 1000) }
      },
    ...
    const averageCarPrice = totalSoldCars > 0 ? totalRevenue / totalSoldCars : 0;
